...
export const rotationSchema = z.object({ x: z.number().finite(), y: z.number().finite(), z: z.number().finite(), w: z.number().finite() }).strict();
...
export const voxelModifySchema = z.object({ chunkKey: z.string().regex(/^(?:-?\d+_-?\d+_-?\d+|ship:[A-Za-z0-9][A-Za-z0-9_-]{0,63})$/), x: z.number().int().min(0).max(15), y: z.number().int().min(0).max(15), z: z.number().int().min(0).max(15), block: z.unknown(), factionId: z.string().uuid().optional() }).strict();
